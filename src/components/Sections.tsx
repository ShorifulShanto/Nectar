...
export function IngredientsSection() {
  ...
  return (
    <section id="ingredients" className="py-24 bg-neutral-950/30">
      <div className="container mx-auto px-6 md:px-12">
        <div className="mb-16 text-center md:text-left">
          <p className="text-white/30 text-[9px] uppercase tracking-[0.4em] mb-3">WHAT'S INSIDE</p>
          <h2 className="text-5xl md:text-6xl font-headline font-bold leading-tight">REAL INGREDIENTS<br />REAL BENEFITS</h2>
          <p className="text-white/40 mt-6 max-w-xl font-light text-base">Every drop starts with real, whole fruits. No concentrates, no preservatives — just nature in a bottle.</p>
        </div>
        
        <div className="grid md:grid-cols-3 gap-6">
          {ingredients.map((item) => (
            <div key={item.name} className="p-8 border border-white/5 bg-black/30 rounded-2xl hover:border-white/20 transition-all duration-500 group">
              <div className="mb-5 text-white/40 group-hover:text-white transition-colors">
                <item.icon size={28} strokeWidth={1} />
              </div>
              <h4 className="text-lg font-headline font-bold mb-3 tracking-widest">{item.name}</h4>
              <p className="text-xs text-white/30 leading-relaxed font-light">{item.benefit}</p>
            </div>
          ))}
        </div>
      </div>
    ...
    <section id="product" className="py-24 bg-black">
      <div className="container mx-auto px-6 md:px-12">
        <div className="mb-16 text-center md:text-left">
          ...
          <h2 className="text-5xl md:text-6xl font-headline font-bold leading-tight">SEVEN FLAVORS<br />ONE OBSESSION</h2>
        </div>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {flavors.map((flavor) => (
            <div key={flavor.id} className="group relative">
               <div className="aspect-[4/5] rounded-3xl bg-neutral-900 border border-white/5 overflow-hidden p-6 mb-4 flex flex-col items-center justify-center group-hover:border-white/10 transition-all duration-700">
                  ...
                  <button 
                    ...
                    className="absolute bottom-5 right-5 w-10 h-10 bg-white text-black rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 shadow-xl"
                  >
                    <Plus size={18} />
                  </button>
               </div>
               <div className="text-center md:text-left">
                 ...
                 <p className="text-[9px] text-white/30 uppercase tracking-widest">$12.00 — 350ml</p>
               </div>
            </div>
          ))}
        </div>
    ...
    <section id="nutrition" className="py-24 bg-black border-y border-white/5">
      <div className="container mx-auto px-6 md:px-12">
        <div className="grid md:grid-cols-2 gap-20 items-start">
          <div className="p-6 border-[4px] border-white rounded-lg max-w-sm mx-auto md:mx-0 shadow-[0_0_30px_rgba(255,255,255,0.05)]">
            <h3 className="text-4xl font-bold font-headline border-b-[8px] border-white pb-1 mb-2 leading-none uppercase">Nutrition</h3>
            <p className="text-[9px] uppercase tracking-widest text-white/40 border-b border-white/20 pb-2 mb-3">Serving Size: 1 Bottle (350ml)</p>
            <div className="flex justify-between items-end border-b-4 border-white pb-2 mb-3">
            ...
            {[
              ...
              <div key={row.label} className={`flex justify-between py-1.5 border-b border-white/10 text-[10px] uppercase tracking-widest ${row.highlight ? 'font-bold text-white' : 'text-white/60'}`}>
                <span className={row.indent ? 'pl-3' : ''}>{row.label}</span>
                <span>{row.val}</span>
              </div>
            ))}
            <p className="text-[8px] text-white/30 mt-3 leading-tight italic">
              * Percent daily values are based on a 2,000 calorie diet.
            </p>
          </div>
          
          <div className="space-y-12">
            <h2 className="text-5xl md:text-6xl font-headline font-bold leading-[0.9] tracking-tighter">NUTRITION<br />YOU CAN FEEL.</h2>
            <div className="grid gap-10">
              {[
                ...
                <div key={item.num} className="flex gap-6 items-start border-l border-white/10 pl-6 group">
                  <span className="text-2xl font-headline font-bold text-white/10 group-hover:text-white/40 transition-colors duration-500">{item.num}</span>
                  <div>
                    <h4 className="text-base font-bold mb-1 uppercase tracking-[0.2em]">{item.title}</h4>
                    <p className="text-white/40 font-light leading-relaxed text-xs">{item.desc}</p>
                  </div>
                </div>
              ))}
            </div>
...
export function ReviewsSection() {
  ...
  return (
    <section id="reviews" className="py-24 bg-neutral-950/30">
      <div className="container mx-auto px-6 md:px-12">
        <div className="mb-16 text-center">
          <p className="text-white/30 text-[9px] uppercase tracking-[0.4em] mb-3">WHAT PEOPLE SAY</p>
          <h2 className="text-5xl md:text-6xl font-headline font-bold uppercase">LOVED BY THOUSANDS</h2>
        </div>
        
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          <div className="bg-black/50 p-8 rounded-3xl border border-white/5 text-center flex flex-col justify-center items-center backdrop-blur-sm">
          ...
          {reviews.map((review, i) => (
            <div key={i} className="p-8 border border-white/5 bg-black/20 rounded-3xl group hover:border-white/20 transition-all duration-500">
              <div className="flex gap-1 mb-5 text-white/20 group-hover:text-white/60 transition-colors">
                {[...Array(review.rating)].map((_, j) => <Star key={j} size={10} fill="currentColor" />)}
              </div>
              <p className="text-base text-white/60 mb-6 leading-relaxed font-light italic">"{review.text}"</p>
    ...
    <section id="faq" className="py-24 bg-black">
      <div className="container mx-auto px-6 md:px-12 max-w-3xl">
        <div className="mb-16 text-center md:text-left">
          ...
          <h2 className="text-4xl md:text-5xl font-headline font-bold uppercase">FREQUENTLY ASKED</h2>
        </div>
        
        <Accordion type="single" collapsible className="space-y-3">
          {faqs.map((faq, i) => (
            <AccordionItem key={i} value={`item-${i}`} className="border-white/5 px-0">
              <AccordionTrigger className="text-base md:text-lg font-headline font-bold hover:no-underline hover:text-white/60 transition-colors py-6 uppercase tracking-widest text-left">
                {faq.q}
              </AccordionTrigger>
              <AccordionContent className="text-white/40 text-sm md:text-base pb-6 leading-relaxed font-light">
                {faq.a}
              </AccordionContent>
            </AccordionItem>
          ))}
      ...
      <div className="container mx-auto px-6 md:px-12">
        <div className="grid md:grid-cols-4 gap-12 mb-12 pb-12 border-b border-white/5">
          <div className="col-span-2">
            <h2 className="text-3xl font-headline font-bold tracking-[0.3em] mb-3">OLLANHO</h2>
            <p className="text-white/20 text-[9px] uppercase tracking-[0.4em]">Fresh Cold-Pressed Juice</p>
          </div>
          <div>
            <h4 className="text-[9px] font-bold text-white/20 uppercase tracking-[0.4em] mb-6">Navigate</h4>
            <ul className="space-y-3 text-[9px] font-bold uppercase tracking-[0.2em] text-white/40">
          ...
          <div>
            <h4 className="text-[9px] font-bold text-white/20 uppercase tracking-[0.4em] mb-6">Contact</h4>
            <ul className="space-y-3 text-[9px] font-bold uppercase tracking-[0.2em] text-white/40">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 text-[8px] text-white/20 uppercase tracking-[0.4em] font-bold">
